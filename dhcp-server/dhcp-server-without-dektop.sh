...
if [ "$(grep '^subnet' $DHCPD)" ]; then
	...
	while [ "$MIN" = "" ] || [ "$MAX" = "" ]; do
		MIN=$(dialog --stdout --inputbox "Rango Mínimo " 8 60)
		MAX=$(dialog --stdout --inputbox "Rango Máxima" 8 60)
		if [ "$MIN" = "" ] || [ "$MAX" = "" ] || [ "$MIN" -ge "$MAX" ] || [ "`echo $MIN | wc -m`" -gt "4" ] || [ "`echo $MAX | wc -m`" -gt "4" ]; then 
			while [ "$MIN" = "" ] || [ "$MAX" = "" ] || [ "$MIN" -ge "$MAX" ] || [ "`echo $MIN | wc -m`" -gt "4" ] || [ "`echo $MAX | wc -m`" -gt "4" ]; do
				MIN=$(dialog --stdout --title "El rango no es válido" --inputbox "Rango Mínimo" 8 60)
				MAX=$(dialog --stdout --inputbox "Rango Máxima" 8 60)
			done
		fi
	done
